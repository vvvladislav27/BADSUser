...
const setAnimationForText = (customClass) => {
    ...
    let cssRules = '';   
    items.forEach((item) => {
        const existingAnimationClass = Array.from(item.classList).find(cls => cls.startsWith('animate-name-'));
        if (existingAnimationClass) {
            item.classList.remove(existingAnimationClass);
        }
